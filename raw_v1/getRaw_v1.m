...
while ishandle(himg)
    ...
    if (trackedBodies ~= 0)
        ...
        rightHandColor = depthMetaData.ColorJointIndices(12,:,trackedBodies);
        ...
        rightHandState(trackedBodies(nearestHand))
        ...
        if snapshotCounter == 2
            ...
            imcrop(depthMap, [rightHand(:,:,nearestHand)-80 160 160]);
            % Save depth image
            savefig(FileName);
            close;
            ...
            figure(2);
            % Save color image
            imcrop(lastColorImage, [rightHandColor(:,:,nearestHand)-200 400 400]);
